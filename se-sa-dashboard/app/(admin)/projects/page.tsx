  ...
  return (
    <div className="grid gap-4">
      <div className="flex items-center justify-between">
        ...
        <a href="/admin/projects/new" className="px-3 py-2 rounded-xl bg-sky-600 hover:bg-sky-500">New</a>
      </div>
      <div className="divide-y divide-slate-800 rounded-2xl border border-slate-800">
        {items.map(i => (
          <a key={i.id} href={"/admin/projects/"+i.id} className="block p-3 hover:bg-slate-900/60">
            <div className="font-medium">{i.title}</div>
            <div className="text-xs opacity-70">{i.status}</div>
          </a>
        ))}
      </div>
    </div>
